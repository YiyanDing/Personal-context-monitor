package com.monitor.android.view;
...
public class ChartView extends View {
    ...
    //x轴各个坐标点水平间距
    private int interval = dpToPx(500);
    //背景颜色
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        if (changed) {
            ...
            int dp2 = dpToPx(2);
            int dp3 = dpToPx(3);
            xOri = (int) (dp2 + textYWdith + dp2 + xylinewidth) + width / 10;//dp2是y轴文本距离左边，以及距离y轴的距离
//            //X轴文本最大高度
...
//        super.onDraw(canvas);
    ...
    private void drawXY(Canvas canvas) {
        ...
        //绘制月份
       // Rect rect = getTextBounds(monthData, xyTextPaint);
        //canvas.drawText(monthData, 0, monthData.length(), xInit - interval * 2 - rect.width() / 2, yOri + xylinewidth + dpToPx(2) + rect.height(), xyTextPaint);
    }
     ...
     * @param moth_data
     */
    public  void setMonth (String moth_data) {
